...
import { userProfileSchema, storeProfileSchema, type UserProfileFormData, type StoreProfileFormData } from "@/schemas/profile";
...
export function ProfileForm({ profile, onProfileChange }: ProfileFormProps) {
  ...
  const schema = isStore ? storeProfileSchema : userProfileSchema;
  const {
    register,
    handleSubmit,
    reset,
    setValue,
    formState: { errors },
  } = useForm<UserProfileFormData | StoreProfileFormData>({
  ...
  const onSubmit = async (data: UserProfileFormData | StoreProfileFormData) => {
    try {
      const updateData = {
        ...
        ...(isStore ? { user_description: (data as StoreProfileFormData).user_description } : {})
      };
          ...
          <div className="grid gap-4">
            ...
            {isStore && (
              <div className="grid gap-2 mt-4 mb-6">
                ...
                <Textarea
                  id="user_description"
                  {...register("user_description" as keyof StoreProfileFormData)}
                ...
                />
                {(errors as FieldErrors<StoreProfileFormData>).user_description && (
                  <p className="text-xs text-destructive">
                    {(errors as FieldErrors<StoreProfileFormData>).user_description?.message}
                  </p>
                )}
              </div>
            )}
